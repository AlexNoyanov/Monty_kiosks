<?php
    ...
    //$txt = $currentDate."|".$s1p."|".$s2p."|".$s3p."|".$s4p."|".$s5p."|".$s6p."|".$s7p."\n";
     $txt = $currentDate."|".$s1p."|".$s2p."|".$s3p."|".$s4p."|".$s5p."|".$s6p."\n";
    //$txt = $currentDate."|".$s1."|".$s2."|".$s3."|".$s4."|".$s5."|".$s6."|".$s7."\n";
    //$filename = $kiosk_id + ".txt";
    
    // Check and create new file to file:
    //if(!is_file($kiosk_id)){
        //$contents = 'This is a test!';           // Some simple example content.
        //file_put_contents($file, $txt);     // Save our content to the file.
    //}else{
